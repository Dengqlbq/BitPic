    ...
    <div class="container g-popup-box">
      ...
      <div class="content">
        ...
        <div class="c-right">
          ...
          <input @change="fileChange" type="file" accept="image/*" style="height: 80px;" />
      ...
      if (this.imgURL) {
        if (this.returnType === 'url') {
          this.$emit('enter', this.$refs.$canvas.toDataURL()) // 返回链接
					...
					var file = this.base64ToFile(this.$refs.$canvas.toDataURL(), 'avatar.png');
					file.url = this.$refs.$canvas.toDataURL();
          this.$emit('enter', file) // 返回文件
        }
      } else {
